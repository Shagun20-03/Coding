...
class Solution{
  public:
    ...
    int evalTree(node* root) {
        ...
        if(root==NULL)  //no operations can be performed
        return 0;
        
        int l=evalTree(root->left); //solve left subtree expression
        int r=evalTree(root->right);    //solve right subtree expression
